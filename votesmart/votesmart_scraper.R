...
# Date: Website accessed on Mar 4, 2021
...
write.csv(candidates,"cand_2022_face.csv", row.names = FALSE)
##########################
df = read_csv("./cand_image_data_2020.csv")

df = df %>% mutate(state_name=toupper(state_name)) 
df$state_name = gsub("[\n]", "", df$state_name)

df %>% left_join(st) %>% 
  mutate(f_name = gsub("[.,]", "", cand_name),
         f_name = gsub(" ", "_", f_name),
         f_name = toupper(f_name)) %>% 
  mutate(file_name = paste0("US", race_code, "_", usps_code, "_", f_name, ".jpg")) %>% 
  mutate(file_name = ifelse(nchar(url) == 0, "", file_name)) %>% 
  select(cand_name, race, race_code, file_name, url,cand_details, state_name) %>% 
  write_csv("cand_image_data_2020_clean.csv")

df2 = read_csv("./cand_image_data_2020_clean.csv")

###########################
df3 = read_csv("./jielu/file_size.csv")
data = df2 %>% left_join(df3) %>% 
  write_csv("cand_image_data_2020_final.csv")
